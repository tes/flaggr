...
function RedisAdapter(config) {
  ...
  this.enable = function(feature, next) {
    this._redisClient.hset(feature, 'enabled', 'true', next);
  };

  this.disable = function(feature, next) {
    this._redisClient.hget(feature, 'enabled', next);
  };
  ...
  this.isEnabled = function(feature, opts, next ) {
    ...
    return this._redisClient.hmget(feature, 'enabled', function(err, reply) {
  ...
  //TODO: Refactor logic of enabled and disable into one method
  this.enableGroup = function(feature, groupName, next) {
    ...
    _this._redisClient.hget(feature, groupKey, function(err, reply) {
      if (err) return next(err);

      var groupObj = JSON.parse(reply);
      groupObj.enabled = true;
  ...
  this.isEnabledForGroup = function(feature, groupName, groupMember, next) {
    ...
    this._redisClient.hmget(feature, ['enabled', groupKey], function(err, replies) {
      var enabled = JSON.parse(replies[0]);
      var group = JSON.parse(replies[1]);
